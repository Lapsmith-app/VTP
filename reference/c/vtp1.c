...
int vtp_decode_power_state(const uint8_t *b, size_t len,
    ...
    /* No range check on `percent`. A value above 100 with its bit set is a
     * device-side violation (SPEC.md 9.7): the record is well formed, so it
     * decodes, and flagging the value is the caller's job -- the same
     * division SPEC.md 1.1 draws for a latitude beyond the pole. The ENCODER
     * refuses it, which is the half of the rule that lives in code. */
    return 0;
}
