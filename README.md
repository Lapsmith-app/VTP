...
- **[harness/](harness/)** — a harness that connects to your device and checks it
...
# Check a real device against the specification, over Bluetooth
uv run vtp1-harness                           # Windows, macOS or Linux
uv run vtp1-harness --loopback                # or against the synthetic device
```
...
harness/                 Conformance harness: connects to YOUR device and tests it
```
...
5. Point [the harness](harness/) at the device: `uv run vtp1-harness`. It reads
   Info, works out which roles you declared, and tests exactly those — including
   the control plane, the shared clock and the reconnect behaviour, none of
   which any byte vector can reach.
